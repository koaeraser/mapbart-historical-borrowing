...
\begin{frame}{A shared surface with local discrepancies}
\centering
\begin{tikzpicture}[
  x=1cm,y=1cm,
  split/.style={draw,rounded corners=1pt,fill=white,minimum size=7pt,inner sep=1pt},
  spike/.style={draw=BayesGreen,rounded corners=2pt,fill=BayesLight,inner sep=5pt,font=\small},
  slab/.style={draw=BayesGold,rounded corners=2pt,fill=BayesGold!12,inner sep=5pt,font=\small},
  edge label/.style={font=\scriptsize,fill=white,inner sep=2pt},
  note/.style={font=\footnotesize,align=left},
  callout/.style={draw=BayesGreen,dashed,thick,rounded corners=3pt,fill=white,inner sep=7pt,align=left,font=\footnotesize}]
...
\draw[BayesGold,dashed,thick,-{Stealth}] (sum.north) -- (middle.south);
...
\node[spike,anchor=north west,text width=5.8cm,align=left,font=\footnotesize] at (8.05,3.70)
  {\textbf{Spike: $z_{h\ell}=1$}\\
   ...
   Draws the RCT mean toward $f(\bx)$.};
\node[slab,anchor=north west,text width=5.8cm,align=left,font=\footnotesize] at (8.05,2.02)
  {\textbf{Slab: $z_{h\ell}=0$}\\
   ...
   Allows departures from $f(\bx)$.};
\node[note,anchor=north west,text width=6cm,font=\scriptsize] at (8.05,0.45)
  {$z_{h\ell}\sim\Ber(w)$. Leaf indicators vary; $w,\tau_0^2,\tau_1^2$ are shared.};
\end{tikzpicture}
\end{frame}
