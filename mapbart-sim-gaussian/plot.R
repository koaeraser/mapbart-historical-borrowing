...
if (sc == 1 | sc == 2){
  ...
  for (subsc in subscenarios) {
    ...
    tryCatch({
      LMv1_res <- readRDS(paste0(mainDir,"/mapbart-sim-gaussian/res/LMv1_p",p_obs,file_suffix))
    ...
    tryCatch({
      LMv2_res <- readRDS(paste0(mainDir,"/mapbart-sim-gaussian/res/LMv2_p",p_obs,file_suffix))
    ...
    tryCatch({
      LMv3_res <- readRDS(paste0(mainDir,"/mapbart-sim-gaussian/res/LMv3_p",p_obs,file_suffix))
    ...
    for (prior_val in lmv4_prior_vals) {
      tryCatch({
        ...
        if (subsc == "NULL") {
          lmv4_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/HierLM_p",p_obs,"_sc",sc,"_prior",prior_val,"_alternative.RData")
        } else {
          lmv4_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/HierLM_p",p_obs,"_sc",sc,subsc,"_prior",prior_val,"_alternative.RData")
        }
    ...
    for (target_N in map_target_Ns) {
      tryCatch({
        ...
        if (subsc == "NULL") {
          map_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/MAP_p",p_obs,"_sc",sc,"_N",target_N,"_alternative.RData")
        } else {
          map_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/MAP_p",p_obs,"_sc",sc,subsc,"_N",target_N,"_alternative.RData")
        }
    ...
    tryCatch({
      PSCL_res <- readRDS(paste0(mainDir,"/mapbart-sim-gaussian/res/PSCL_p",p_obs,file_suffix))
    ...
    tryCatch({
      BART_res <- readRDS(paste0(mainDir,"/mapbart-sim-gaussian/res/",bver,"_p",p_obs,file_suffix))
    ...
    # Try to read mBART results for each N_target value
    for (target_N in mbart_target_Ns) {
      tryCatch({
        # Construct file path with target N
        if (subsc == "NULL") {
          mbart_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,"_N",target_N,"_alternative.RData")
        } else {
          mbart_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,subsc,"_N",target_N,"_alternative.RData")
        }
        ...
        if (file.exists(mbart_file)) {
          ...
          mBART_res_sigma <- mBART_res[, c("c","iteration",
                                                ...
                                                "bias.ctrl.median.pop","w2distance.ctrl.median.pop",
                                                "pehe_subj","bias_subj")]
          mBART_res_ATE$Method <- paste0("MAP-BART(N=", target_N, ")")
          mBART_res_sigma$Method <- paste0("MAP-BART(N=", target_N, ")")
  ...
  for (subsc in subscenarios) {
    ...
    methods_list <- list(
      ...
      list(name = "mBART", file_prefix = "MAP-BART")
    )

    for (method_info in methods_list) {
      tryCatch({
        null_file <- paste0(mainDir,"/mapbart-sim-gaussian/res/", method_info$file_prefix, "_p", p_obs, null_file_suffix)
        if (file.exists(null_file)) {
          null_res <- readRDS(null_file)

          # For BART and mBART, filter by alpha and beta
          if (method_info$name %in% c("BARTv1", "BARTv2", "BARTv3", "mBART")) {
...
if (sc == 3){
  ...
  for (subsc in subscenarios) {
    for (cor_val in cor_vals) {
      ...
      # Try to read LMv1 results
      tryCatch({
        file_path <- paste0(mainDir,"mapbart-sim-gaussian/res/LMv1_p",p_obs,file_suffix)
      ...
      # Try to read LMv2 results
      tryCatch({
        file_path <- paste0(mainDir,"mapbart-sim-gaussian/res/LMv2_p",p_obs,file_suffix)
        if (file.exists(file_path)) {
          LMv2_res <- readRDS(file_path)
          LMv2_res_ATE <- LMv2_res[, c("c","iteration","bias","sd","rmse","w1distance","w2distance","ci","coverage","tp","fp","tp_calibrated")]
          LMv2_res_sigma <- LMv2_res[, c("c","iteration",
                                        "bias.trt.sigma","sd.trt.sigma","w2distance.trt.sigma",
                                        "bias.ctrl.sigma","sd.ctrl.sigma","w2distance.ctrl.sigma",
      ...
      # Try to read LMv3 results
      tryCatch({
        file_path <- paste0(mainDir,"mapbart-sim-gaussian/res/LMv3_p",p_obs,file_suffix)
        if (file.exists(file_path)) {
      ...
      for (prior_val in lmv4_prior_vals) {
        tryCatch({
          # Construct file path with prior
          if (subsc == "NULL") {
            lmv4_file <- paste0(mainDir,"mapbart-sim-gaussian/res/HierLM_p",p_obs,"_sc",sc,"_cor",cor_val,"_prior",prior_val,"_alternative.RData")
          } else {
            lmv4_file <- paste0(mainDir,"mapbart-sim-gaussian/res/HierLM_p",p_obs,"_sc",sc,subsc,"_cor",cor_val,"_prior",prior_val,"_alternative.RData")
          }
      ...
      for (target_N in map_target_Ns) {
        tryCatch({
          # Construct file path with target N
          if (subsc == "NULL") {
            map_file <- paste0(mainDir,"mapbart-sim-gaussian/res/MAP_p",p_obs,"_sc",sc,"_cor",cor_val,"_N",target_N,"_alternative.RData")
          } else {
            map_file <- paste0(mainDir,"mapbart-sim-gaussian/res/MAP_p",p_obs,"_sc",sc,subsc,"_cor",cor_val,"_N",target_N,"_alternative.RData")
          }
      ...
      # Keep all rows including NAs - plotting functions will handle with na.rm = TRUE
      tryCatch({
        file_path <- paste0(mainDir,"mapbart-sim-gaussian/res/PSCL_p",p_obs,file_suffix)
        if (file.exists(file_path)) {
      ...
      for (bver in c("BARTv1", "BARTv2", "BARTv3")) {
      tryCatch({
        file_path <- paste0(mainDir,"mapbart-sim-gaussian/res/",bver,"_p",p_obs,file_suffix)
        if (file.exists(file_path)) {
      ...
      for (target_N in mbart_target_Ns) {
        tryCatch({
          # Construct file path with target N
          if (subsc == "NULL") {
            mbart_file <- paste0(mainDir,"mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,"_cor",cor_val,"_N",target_N,"_alternative.RData")
          } else {
            mbart_file <- paste0(mainDir,"mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,subsc,"_cor",cor_val,"_N",target_N,"_alternative.RData")
          }
...
all_method_levels <- c("LMv1", "LMv2", "LMv3", "BARTv1", "BARTv2", "BARTv3", "PSCL", map_methods, lmv4_methods, mbart_methods)
...
if (sc == 1 | sc ==2){
  ...
  ggsave(paste0(file.path(mainDir),"/mapbart-sim-gaussian/inserts/p",p_obs,"_sc",sc,"_all_subsc_results.jpg"),
         width = total_width,
         height = total_height,
         final_plot,
         limitsize = FALSE)
