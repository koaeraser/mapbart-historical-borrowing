...
if (length(.cal_files)) {
  ...
  cat(sprintf("mBART.R: from %s\n", basename(.cal_latest)))
  for (k in seq_along(prior_vals))
    cat(sprintf("  N=%d -> prior(s^2) = %.4g\n",
                target_Ns[k], prior_vals[k]))
} else {
  # Fallback when no sc-matched ess calibration exists: non-_real folders use
  # s^2 = 0.25 (paired with the 1e-4 / N=Inf reference appended below).
  prior_vals <- 0.25
  target_Ns  <- 100L
  cat(sprintf("mBART.R: no sc-matched calibration -> fallback s^2 = %.4g (target_N=%d)\n",
...
cat(sprintf("mBART.R: s^2 sweep (calibrated s^2_star + 0.0001) = %s\n", paste(prior_vals, collapse=", ")))
...
if (hypo == "alternative") {
  ...
  if (sc == 1 | sc == 2) {
    if (is.null(subsc)) {
      threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/mBART_p", p_obs, "_sc", sc, "_threshold.RData")
    } else {
      threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/mBART_p", p_obs, "_sc", sc, subsc, "_threshold.RData")
    }
  } else {
    if (is.null(subsc)) {
      threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/mBART_p", p_obs, "_sc", sc, "_cor", cor, "_threshold.RData")
    } else {
      threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/mBART_p", p_obs, "_sc", sc, subsc, "_cor", cor, "_threshold.RData")
    }
  }
...
for (.idx in seq_along(prior_vals)) {
    ...
    for (c in cor){
      for (H in ntree){
        for (iter in 1:niter){
          ...
          t=2
          tau = (max(y.train)-min(y.train))/(2*t*sqrt(ntree))
  ...
  if (hypo == "null") {
    ...
    if (sc == 1 | sc == 2) {
      if (is.null(subsc)) {
        threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/MAP-BART_p", p_obs, "_sc", sc, "_threshold.RData")
      } else {
        threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/MAP-BART_p", p_obs, "_sc", sc, subsc, "_threshold.RData")
      }
    } else {
      if (is.null(subsc)) {
        threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/MAP-BART_p", p_obs, "_sc", sc, "_cor", c, "_threshold.RData")
      } else {
        threshold_file <- paste0(mainDir, "/mapbart-sim-gaussian/res/MAP-BART_p", p_obs, "_sc", sc, subsc, "_cor", c, "_threshold.RData")
      }
    }
  ...
  if (sc == 1 | sc == 2) {
    if (is.null(subsc)) {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,"_N",target_N,"_",hypo,".RData"))
    } else {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,subsc,"_N",target_N,"_",hypo,".RData"))
    }
  } else {
    if (is.null(subsc)) {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,"_cor",c,"_N",target_N,"_",hypo,".RData"))
    } else {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-gaussian/res/MAP-BART_p",p_obs,"_sc",sc,subsc,"_cor",c,"_N",target_N,"_",hypo,".RData"))
    }
  }
