...
##
## Companion to ess_calibration_map.R (vanilla MAP, no BART): the two
## scripts share CLI flags, section order, and the same Stage-2 gMAP +
## automixfit + ELIR ESS core; this script differs only in what it
## supplies as the "leaf" summary.  Run both on the same RWD to compare
## MAP-BART vs vanilla-MAP calibration on identical settings.
##
...
##
## CLI is a strict superset of ess_calibration_map.R: the flags up
## through --sigma-ref are shared (same order, possibly different
## defaults); --B, --burn, --q are BART-only.
##
## --map-model selects which RBesT_lite/stan/gMAP_*.stan is fit per leaf:
##   hybrid (default)    -- two-arm hybrid design: theta_h ~ N(0, tau^2),
...
##   <numeric>          -- fixed value for every fit (parallels the map
##                          script's --sigma-ref)
##
...
map_model  <- get_arg("--map-model", "hybrid")
...
## --- BART-only flags (no analog in ess_calibration_map.R) ---
...
## the source RWD file so different scenarios never clobber each other.
...
.stale  <- file.path(.resDir, c(
  sprintf("ess_calibration_gaussian_%s%s.RData",            map_model, sc_suffix),
  sprintf("ess_calibration_gaussian_%s%s_checkpoint.RData", map_model, sc_suffix)))
...
## (parallels the map script's --sigma-ref behaviour).
...
## map .RData and produces a single merged figure.
