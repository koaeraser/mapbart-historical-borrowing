...
public:
   ...
   // between drmu() and the re-fit, exposing the BART partial residual
   // r[k] and the just-updated tree t[j] to caller code.
   void draw_per_tree(size_t j, double sigma, rn& gen) {
   ...
   //----- Calibration-hook accessors (read-only views).
   double  getr(size_t k)     const {return r[k];}
   double* getx_row(size_t k)       {return &x[k*p];}
   size_t  getp_()            const {return p;}
   size_t  getn_()            const {return n;}
   xinfo&  get_xi()                 {return xi;}
protected:
