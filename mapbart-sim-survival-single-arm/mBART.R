...
# Calibrated borrowing targets (N and the matching leaf-prior s^2) are
# EXTRACTED ONCE by ess_local/ess_plot.R, which writes a per-calibration table
# ess_calibration_<version>_tab.RData -- data.frame(N, s2, multiplier) --
# alongside the calibration PNG.  mBART.R no longer recomputes the Pr-rule; it
# just READS the size-/config-/sc-matched targets table (pick_prior_for below).
# Run ess_plot.R on the calibration BEFORE running mBART.R.
...
.cal_files <- list.files(.cal_dir,
                           ...
                           full.names = TRUE)
# Drop pre-aggregate checkpoints and the *_tab.RData tables (ess_plot output)
# so only true calibration files are matched below.
.cal_files <- .cal_files[!grepl("_(checkpoint|tab)\\.RData$", .cal_files)]
## Restrict to the calibration matching THIS run's RWD size.  The ess_local
## outputs are size-tagged (ess_calibration_<version>_n30.RData / _n200.RData)
## via --outcome, mirroring size_suffix here.  Without this filter the
## "latest by mtime" pick can grab the OTHER size's calibration (e.g. an n200
## file for an n30 run), mislabel target_Ns (200/150/100 instead of 30/22/15),
## and then only the Inf reference run lines up at plot time.
.size_tag  <- sub("^_", "", size_suffix)                    # "n30" or "n200"
## Scenario tag mirrors the suffix ess_calibration.R appends AFTER the size tag
...
## structure (see ess_local/ess_calibration.R:97-104), so a config's s^2 is not
## transferable to another config.  The config tag mirrors ess_calibration.R's
## cfg_tag = sprintf("_nt%d_k%g", ntree, k) and is REQUIRED here (previously it
## was optional, so all configs silently inherited whichever single calibration
## file was newest by mtime -- e.g. nt10_k2 / nt50_k2 using nt5_k2's s^2).
## Called inside the control-config loop so prior_vals/target_Ns are per-config.
pick_prior_for <- function(ctrl_ntree, ctrl_k) {
  cfg_tag   <- sprintf("nt%d_k%g", ctrl_ntree, ctrl_k)
  cal_match <- .cal_files[grepl(paste0("_", .size_tag, "_", cfg_tag, "_", .sc_cal_tag, "\\.RData$"),
                                basename(.cal_files))]
  if (length(cal_match)) {
    cal_latest <- cal_match[order(file.info(cal_match)$mtime, decreasing = TRUE)][1]
    # Read the calibrated (N, s^2) targets ess_plot.R wrote next to this
    # calibration -- do NOT recompute the Pr-rule here (single source of truth).
    tgt_path <- sub("\\.RData$", "_tab.RData", cal_latest)
    if (!file.exists(tgt_path))
      stop(sprintf(paste0("mapbart-sim-survival-single-arm/mBART.R: [%s] targets table %s is missing.\n",
                          "  Run ess_local/ess_plot.R on %s first (it writes the (N, s^2) table)."),
                   cfg_tag, basename(tgt_path), basename(cal_latest)))
    tgt <- readRDS(tgt_path)
    pv  <- tgt$s2                      # leaf-prior variance per target
    tn  <- as.integer(tgt$N)           # integer target N (used in filenames)
    cat(sprintf("mapbart-sim-survival-single-arm/mBART.R: [%s] from %s (size %s)\n",
                cfg_tag, basename(tgt_path), .size_tag))
    for (j in seq_along(pv))
      cat(sprintf("  N=%d -> prior(s^2) = %.4g\n", tn[j], pv[j]))
  } else {
    # Fallback when no size-/config-/sc-matched ess calibration exists.
    pv <- 0.05
    tn <- 100L
    cat(sprintf(paste0("mapbart-sim-survival-single-arm/mBART.R: [%s] WARNING -- no size-/config-/sc-matched ",
                "calibration (ess_calibration_*_%s_%s_%s.RData) in %s.\n  Using fallback ",
                "s^2 = %.4g (target_N=%d) + the 1e-4 (N=Inf) reference.  Run ess_calibration ",
                "with --ntree %d --k %g first for calibrated targets.\n"),
                cfg_tag, .size_tag, cfg_tag, .sc_cal_tag, .cal_dir, pv, tn, ctrl_ntree, ctrl_k))
  }
  ...
  pv <- c(pv, 0.0001)
  tn <- c(tn, Inf)
  cat(sprintf("mapbart-sim-survival-single-arm/mBART.R: [%s] s^2 sweep (calibrated s^2_star + 0.0001) = %s\n",
...
# Only run configs that have a size-/config-/sc-matched ess calibration file
# (same required tag as pick_prior_for); skip the rest rather than running them
# on the uncalibrated fallback s^2.
has_cal_for <- function(ctrl_ntree, ctrl_k) {
  cfg_tag <- sprintf("nt%d_k%g", ctrl_ntree, ctrl_k)
  cal_match <- .cal_files[grepl(paste0("_", .size_tag, "_", cfg_tag, "_", .sc_cal_tag, "\\.RData$"),
                                basename(.cal_files))]
  # Require BOTH the calibration AND the ess_plot.R targets table: without the
  # table there is nothing to read (see pick_prior_for), so skip such configs
  # rather than hard-error mid-sweep.  Run ess_plot.R to produce the table.
  length(cal_match) > 0 && any(file.exists(sub("\\.RData$", "_tab.RData", cal_match)))
}
.has_cal <- vapply(control_configs, function(cf) has_cal_for(cf$ntree, cf$k), logical(1))
if (any(!.has_cal))
  cat(sprintf("mBART.R: skipping control configs with no %s/%s calibration: %s\n",
              .size_tag, .sc_cal_tag,
...
if (length(control_configs) < 1) {
  ...
  warning(sprintf("mBART.R: no control config has a matching %s/%s ess calibration (ess_calibration_*_%s_nt<ntree>_k<k>_%s.RData in %s) -- SKIPPING MAP-BART for this scenario. Run ess_calibration for this sc/config first.",
                  .size_tag, .sc_cal_tag, .size_tag, .sc_cal_tag, .cal_dir))
} else {
...
# Single-arm: external-control-only build of cmbart (mBART.real), mirroring
# mapbart-case-study-mm/mBART_analysis.R; treatment arm uses aBART (cabart).
...
for (.idx in seq_along(prior_vals)) {
  ...
  if (hypo == "alternative") {
    ...
    if (sc == 1 | sc == 2) {
      if (is.null(subsc)) {
        threshold_file <- paste0(mainDir, "/mapbart-sim-survival-single-arm/res/MAP-BART_p", p_obs, size_suffix, "_nt", ctrl_ntree, "_k", ctrl_k, "_sc", sc, "_prior", prior, "_threshold.RData")
      } else {
        threshold_file <- paste0(mainDir, "/mapbart-sim-survival-single-arm/res/MAP-BART_p", p_obs, size_suffix, "_nt", ctrl_ntree, "_k", ctrl_k, "_sc", sc, subsc, "_prior", prior, "_threshold.RData")
      }
    } else {
      if (is.null(subsc)) {
        threshold_file <- paste0(mainDir, "/mapbart-sim-survival-single-arm/res/MAP-BART_p", p_obs, size_suffix, "_nt", ctrl_ntree, "_k", ctrl_k, "_sc", sc, "_cor", cor, "_prior", prior, "_threshold.RData")
      } else {
        threshold_file <- paste0(mainDir, "/mapbart-sim-survival-single-arm/res/MAP-BART_p", p_obs, size_suffix, "_nt", ctrl_ntree, "_k", ctrl_k, "_sc", sc, subsc, "_cor", cor, "_prior", prior, "_threshold.RData")
      }
    }
    ...
    for (c in cor){
      for (H in ntree){
        for (iter in 1:niter){
          ...
          # RCT group (s==1). Single-arm has no concurrent control, so the s==1
          # group is empty; cmbart (mBART.real) ignores the s==1 prior in that
          # case, so fall back to sigest0 to keep sigest1/sigscale1 finite.
          if (sum(s.train == 1) > 0) {
          ...
          k=ctrl_k
          tau = (max(y.train)-min(y.train))/(2*ctrl_k*sqrt(ctrl_ntree))
          ...
          median_survival_ctrl <- pop_median_draws(res_ctrl$f.test, res_trt$sigma)
          ...
          .rm <- compute_rmst_metrics(res_trt$yhat.test, res_trt$sigma,
                                      res_ctrl$f.test, tail(res_ctrl$sigma_rwd, length(res_trt$sigma)),
  ...
  if (hypo == "null") {
    ...
    if (sc == 1 | sc == 2) {
      ...
      if (is.null(subsc)) {
        threshold_file <- paste0(mainDir, "/mapbart-sim-survival-single-arm/res/MAP-BART_p", p_obs, size_suffix, "_nt", ctrl_ntree, "_k", ctrl_k, "_sc", sc, "_cor", c, "_prior", prior, "_threshold.RData")
      } else {
        threshold_file <- paste0(mainDir, "/mapbart-sim-survival-single-arm/res/MAP-BART_p", p_obs, size_suffix, "_nt", ctrl_ntree, "_k", ctrl_k, "_sc", sc, subsc, "_cor", c, "_prior", prior, "_threshold.RData")
      }
    }
  ...
  if (sc == 1 | sc == 2) {
    if (is.null(subsc)) {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-survival-single-arm/res/MAP-BART_p",p_obs,size_suffix,"_nt",ctrl_ntree,"_k",ctrl_k,"_sc",sc,"_N",target_N,"_",hypo,".RData"))
    } else {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-survival-single-arm/res/MAP-BART_p",p_obs,size_suffix,"_nt",ctrl_ntree,"_k",ctrl_k,"_sc",sc,subsc,"_N",target_N,"_",hypo,".RData"))
    }
  } else {
    if (is.null(subsc)) {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-survival-single-arm/res/MAP-BART_p",p_obs,size_suffix,"_nt",ctrl_ntree,"_k",ctrl_k,"_sc",sc,"_cor",c,"_N",target_N,"_",hypo,".RData"))
    } else {
      saveRDS(res, file=paste0(mainDir,"/mapbart-sim-survival-single-arm/res/MAP-BART_p",p_obs,size_suffix,"_nt",ctrl_ntree,"_k",ctrl_k,"_sc",sc,subsc,"_cor",c,"_N",target_N,"_",hypo,".RData"))
    }
  }
