  ...
  if (sc == 1 | sc == 2) {
    for (subsc in subscenarios) {
      for (c in cor) {
        for (hypo in c("null", "alternative")) {
          if (subsc == "NULL") {
            filename_rct <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,"_",hypo,"_",iter_rct,".RData")
            filename_rwd <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,"_",hypo,"_",iter_rwd,".RData")
          } else {
            filename_rct <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,subsc,"_",hypo,"_",iter_rct,".RData")
            filename_rwd <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,subsc,"_",hypo,"_",iter_rwd,".RData")
          }
    ...
    for (subsc in subscenarios) {
      for (c in cor) {
        for (hypo in c("null", "alternative")) {
          if (subsc == "NULL") {
            filename_rct <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,"_cor",c,"_",hypo,"_",iter_rct,".RData")
            filename_rwd <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,"_cor",c,"_",hypo,"_",iter_rwd,".RData")
          } else {
            filename_rct <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,subsc,"_cor",c,"_",hypo,"_",iter_rct,".RData")
            filename_rwd <- paste0(mainDir,"/mapbart-sim-survival/",data_folder,"/data_p",p_obs,"_sc",sc,subsc,"_cor",c,"_",hypo,"_",iter_rwd,".RData")
          }
...
p_km_final <- wrap_plots(km_plots, ncol = 5)

# Save KM survival plot
output_suffix <- ifelse(data_folder == "data", "", paste0("_", data_folder))
ggsave(paste0(file.path(mainDir),"/mapbart-sim-gaussian/manuscript/inserts/balanced_1_surv.jpg"),
       width = 16,
       height = 3,
       p_km_final,
       limitsize = FALSE)
...
ggsave(paste0(file.path(mainDir),"/mapbart-sim-gaussian/manuscript/inserts/balanced_2_surv.jpg"),
       width = 14,
       height = plot_height,
       p_balance,
       limitsize = FALSE)
